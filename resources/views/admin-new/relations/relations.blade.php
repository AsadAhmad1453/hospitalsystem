                    ...
                    <div class="card-body p-4">
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                ...
                                <p class="text-muted mb-0">Manage dependencies and relationships between questions and their
                                    options.</p>
                            </div>
        ...
        <div class="row mb-4">
            @php
                $statsCards = [
                    ...
                    [
                        'icon' => 'fas fa-link',
                        'number' => \App\Models\Dependency::count(),
                        'label' => 'Active Relations',
                        'style' => 'background: linear-gradient(135deg, #28a745 0%, #20c997 100%);',
                    ],
                    [
                        'icon' => 'fas fa-layer-group',
                        'number' => $questions->groupBy('section_id')->count(),
                        'label' => 'Sections',
                        'style' => 'background: linear-gradient(135deg, #17a2b8 0%, #6f42c1 100%);',
                    ],
                    [
                        'icon' => 'fas fa-cogs',
                        'number' => $questions->where('question_type', 0)->count(),
                        'label' => 'Single Choice', // Corrected based on switch statement in file
            ...
            @foreach ($statsCards as $card)
                <div class="col-xl-3 col-md-6 mb-4">
                    <div class="card stats-card h-100"
                        @if ($card['style']) style="{{ $card['style'] }}" @endif>
                        <div class="card-body d-flex align-items-center">
                            <div class="flex-grow-1">
                                <div class="d-flex align-items-center">
                                    <div class="icon me-2">
                                        <i class="{{ $card['icon'] }} fa-xs"></i>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        <div class="number stat-number h6">{{ $card['number'] }}</div>
                                        <div class="label " style="font-size: 12px !important;">&nbsp;{{ $card['label'] }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                @if ($question->description)
                                                    <small
                                                        class="text-muted">{{ Str::limit($question->description, 50) }}</small>
                                                @endif
                                            </td>
                                            <td>
                                                <span
                                                    class="badge bg-info">{{ $question->section->name ?? 'No Section' }}</span>
                                            </td>
                                            <td>
                                                ...
                                                @if ($question->options && $question->options->count() > 0)
                                                    <div class="d-flex flex-wrap gap-1">
                                                        @foreach ($question->options->take(3) as $option)
                                                            <span
                                                                class="badge bg-light text-dark">{{ $option->option }}</span>
                                                        @endforeach
                                                        @if ($question->options->count() > 3)
                                                            <span
                                                                class="badge bg-secondary">+{{ $question->options->count() - 3 }}
                                                                more</span>
                                                        @endif
                                                    </div>
                                                @else
                                            ...
                                            <td>
                                                @php
                                                    $dependencies = \App\Models\Dependency::where(
                                                        'question_id',
                                                        $question->id,
                                                    )->count();
                                                @endphp
                                            ...
                                            <td>
                                                <div class="dropdown position-static">
                                                    ...
                                                    <ul class="dropdown-menu show-overflow-menu"
                                                        style="position: absolute; z-index: 1050; min-width: 200px;">
                                                        <li><a class="dropdown-item" href="#"
                                                                onclick="viewRelations({{ $question->id }})">
                                                                <i class="fas fa-eye me-2"></i>View Relations
                                                            </a></li>
                                                        <li><a class="dropdown-item" href="#"
                                                                onclick="editRelations({{ $question->id }})">
                                                                <i class="fas fa-edit me-2"></i>Edit Relations
                                                            </a></li>
                                                        <li><a class="dropdown-item" href="#"
                                                                onclick="addRelation({{ $question->id }})">
                                                        ...
                                                        <li><a class="dropdown-item text-danger" href="#"
                                                                onclick="deleteAllRelations({{ $question->id }})">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-control select2" id="selected_question_id" name="selected_question_id"
                                    required>
        ...
        function viewRelations(questionId) {
            ...
            `;
                    ...
                    if ((response.relations || []).length > 0) {
                        ...
                        content +=
                            '<thead><tr><th>Option</th><th>Related Question</th><th>Actions</th></tr></thead><tbody>';

                        response.relations.forEach(function(relation) {
                            const optText = relation.option?.option ?? relation.option?.option_text ??
                                '';
                            const relQ = relation.related_question?.question ?? relation
