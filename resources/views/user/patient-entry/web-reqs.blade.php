            ...
            <div class="col-12">
                <div class="card">
                    <table class="datatables-basic table">
                        ...
                        <tbody>
                            @foreach($webreqs as $webreq)
                                <tr class="text-center">
                                    <td>{{ $webreq->name }}</td>
                                    <td>{{ $webreq->phone }}</td>
                                    <td>{{ $webreq->date }}</td>
                                    <td>{{ $webreq->services }}</td>
                                    <td class="text-center">
                                        <a href="{{route('del-user-patient', $webreq->id)}}" data-jobs="sdadas" class="text-danger course-sure"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
...
<script>
    ...
    if (dt_basic_table.length) {
        ...
        $('div.head-label').html('<h4 class="mb-0 pl-1 "><strong>Website Requests </strong> </h4>');
    }
