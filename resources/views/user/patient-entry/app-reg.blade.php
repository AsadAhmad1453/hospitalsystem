                            ...
                            @foreach ($patients as $patient)
                                <tr class="text-center">
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('register-patient', $patient->id) }}"
                                            class="text-info confirm-register" title="Register">
    ...
    <script>
        @if (session('success'))
            Swal.fire({
                ...
                html: `
                <p>{{ session('success') }}</p>
                @if (session('new_patient_email'))
                    <div style="text-align: left; margin-top: 10px;">
                        <hr>
                        <p class="mb-0"><strong>Email:</strong> {{ session('new_patient_email') }}</p>
                        <p class="mb-0"><strong>Password:</strong> {{ session('new_patient_password') }}</p>
                        <small class="text-muted">(Password is patient's CNIC without dashes)</small>
                    </div>
                @endif
            `,
        ...
        $(document).on('click', '.confirm-register', function(event) {
            ...
            Swal.fire({
                ...
                text: "This will create a user account for the patient.",
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
