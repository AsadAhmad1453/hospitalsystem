...
@section('content')
                ...
                <div class="dc-shell">
                    <form action="{{ route('save-data-collector', $questions->first()->form_id) }}" method="POST">
                        ...
                        @foreach ($sections as $index => $section)
                            <div class="dc-section-panel" id="section-block-{{ $index }}"
                                data-section-name="{{ $section->name }}" style="{{ $index === 0 ? '' : 'display:none;' }}">
                                <div class="question-stack">
                                    @foreach ($questions->where('section.id', $section->id) as $question)
                                        <div class="dc-question {{ $question->priority == 1 ? 'required' : '' }}"
                                            id="question-{{ $question->id }}">
                                            <div class="d-flex justify-content-between flex-wrap gap-2 mb-3">
                                                <p class="mb-0 dc-question-title">{{ $question->question }}</p>
                                                <span class="badge badge-light text-uppercase">
                                                    {{ $question->question_type == '0' ? 'Single' : ($question->question_type == '1' ? 'Multi' : 'Input') }}
                                                </span>
                                            </div>
                                            ...
                                            @elseif($question->question_type == '2')
                                                <input type="text" class="form-control dc-line-input"
                                                    name="{{ $question->id }}" placeholder="Type your answer..."
                                                    autocomplete="off">
                                            @elseif($question->question_type == '3')
                                                <input type="date" class="form-control dc-line-input "
                                                    name="{{ $question->id }}" max="{{ date('Y-m-d') }}">
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="dc-nav">
                            ...
                            <div class="ml-auto d-flex gap-2">
                                <button type="button" id="next-btn" class="dc-btn dc-btn-primary"
                                    style="{{ $totalSteps < 2 ? 'display:none;' : '' }}">Next section</button>
                                <button id="submit-btn" type="submit" class="dc-btn dc-btn-primary"
                                    style="{{ $totalSteps > 1 ? 'display:none;' : '' }}">Submit responses</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <div class="dc-recorder">
        <button id="recordBtn" type="button" class="btn btn-danger btn-sm rounded-pill px-3"><i
                data-feather="mic"></i></button>
        <button id="pauseBtn" type="button" class="btn btn-secondary btn-sm rounded-pill px-3" disabled><i
                data-feather="pause"></i></button>
        <button id="stopBtn" type="button" class="btn btn-dark btn-sm rounded-pill px-3" disabled><i
...
@section('custom-js')
    <script>
        // Validates the currently visible step and toggles Next/Submit buttons
        function validateActiveStep() {
            const $activePanel = $('.dc-section-panel:visible');
            if ($activePanel.length === 0) return;

            let isValid = true;
            $activePanel.find('.dc-question.required:visible').each(function() {
                const $el = $(this);
                // Check for radio/checkbox groups
                if ($el.find('input[type="radio"], input[type="checkbox"]').length > 0) {
                    if ($el.find('input:checked').length === 0) isValid = false;
                } else {
                    // Check for text/date inputs
                    const $input = $el.find('input');
                    if ($input.length > 0 && !$input.val()) isValid = false;
                }
            });
        ...
        function handleSingleSelectChange(questionId, optionId) {
            ...
            if (isCheckbox) {
                ...
                if (hiddenQuestionsTracker[questionId]) {
                    ...
                    validateActiveStep(); // Re-validate after showing
                }
                    ...
                    checkedOptions.each(function() {
                        ...
                        checkDependencyForOption(questionId, optId, function(hiddenQuestions) {
                            ...
                            if (processedCount === checkedCount) {
                                ...
                                validateActiveStep(); // Re-validate after hiding
                            }
                        });
                    });
                } else {
                ...
                checkDependencyForOption(questionId, optionId, function(hiddenQuestions) {
                    ...
                    validateActiveStep(); // Re-validate after hiding/processing
                });
            }
        }
        ...
        function checkDependencyForOption(questionId, optionId, callback) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.questions_in_range && response.questions_in_range.length >
                        0) {
    ...
    <script>
        ...
        $(function() {
            ...
            function showStep(index) {
                ...
                validateActiveStep(); // Check validity when showing step
            }

            // Keep validating on input changes
            $(document).on('change input keyup', '.dc-section-panel input, .dc-section-panel select', function() {
                validateActiveStep();
            });
            ...
            $('#next-btn').on('click', function(e) {
                e.preventDefault();
                if ($(this).attr('disabled')) return; // Extra safety
