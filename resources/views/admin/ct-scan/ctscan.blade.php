    ...
    <section id="basic-datatable">
            ...
            <div class="col-12">
                <div class="card">
                    <table class="datatables-basic table">
                        ...
                        <tbody>
                            @foreach($ctscans as $scan)
                                <tr>
                                    <td>{{$loop->index+1}}</td>
                                    <td>{{$scan->name}}</td>
                                    <td>
                                        <a href="{{route('del-ctscan', $scan->id)}}" data-jobs="sdadas" class="text-danger course-sure"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="modal modal-slide-in fade" id="modals-slide-in">
            <div class="modal-dialog sidebar-sm">
                <form action="{{route('save-ctscan')}}" id="form" method="POST" class="add-new-record modal-content pt-0">
                    @csrf
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">×</button>
                    <div class="modal-header mb-1">
                        <h5 class="modal-title" id="exampleModalLabel">New CT Scan</h5>
                    </div>
                    <div class="modal-body flex-grow-1">
                        <div class="form-group">
                            <label class="form-label" for="name">Name</label>
                            <input type="text" name="name" class="form-control dt-full-name @error('name') is-invalid @enderror" id="name" placeholder="Name" aria-label="John Doe" />
                            @error('name')
...
<script>
    ...
    $(function () {
        ...
        if (dt_basic_table.length) {
            ...
            $('div.head-label').html('<h6 class="mb-0">CT Scans</h6>');
        }
    });
    ...
    Swal.fire({
        icon: 'warning',
        title: 'Are you sure?',
        text: "You want to remove this CT Scan!",
